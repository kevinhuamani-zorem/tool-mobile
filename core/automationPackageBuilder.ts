...
const recordedPlatform=scenario.platform;const declaredCompletions=new Set((response.completions||[]).filter(c=>c.platform===recordedPlatform).map(c=>c.name));
for(const mod of (reuse.elements||[])){const block=(mod.groups||{})[recordedPlatform];if(!block)continue;for(const el of (mod.elements||[])){const slot=(el.locators||{})[recordedPlatform];if(!slot||slot.status!=='missing')continue;const referenced=new RegExp('\\b'+mod.identifier+'\\s*\\.\\s*'+block+'\\s*\\.\\s*'+el.name+'\\b').test(screen);if(referenced&&!declaredCompletions.has(el.name))errors.push('Cobertura de plataforma: '+mod.identifier+'.'+block+'.'+el.name+' no tiene valor en '+recordedPlatform+'. Rellenalo declarandolo en completions con la accion que lo capturo, o usa un locator del modulo de este caso.')}}
for(const c of (response.completions||[])){if(!(scenario.actions||[]).some(a=>a.sequence===c.sequence&&a.selector))errors.push('completions apunta a la accion '+c.sequence+', que no capturo ningun elemento')}
