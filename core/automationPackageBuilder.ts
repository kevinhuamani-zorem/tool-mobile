...
import { emptyQueryResults, queryRequestsSchema } from './agentQueryContracts';
...
function instructions(result: ResolverResult): string {
    ...
    return `# Contrato del agente de automatización\n\n` +
        ...
        `- Todo lo que necesitas está en esta carpeta. Trabaja únicamente con los archivos del manifiesto, usando sus nombres relativos. Si algo te falta, escribe agent-response.json con decision:'unresolved' y di en reason qué te faltó.\n` +
        `- Si te falta información, NO la busques fuera de esta carpeta: declárala en agent-response.json como unresolved con reason claro y, si aplica en PASS 2, incluye needs para que el orquestador te la entregue.\n` +
        `- En tu contexto no existe ninguna ruta al framework, y no la necesitas. Todo lo del framework llega ya resuelto: los contratos y helpers en framework-api.json, y el código concreto en query-results.json. Si necesitas algo del framework que no esté ahí, es un gap: escríbelo en agent-response.json como unresolved y di qué te faltó.\n` +
