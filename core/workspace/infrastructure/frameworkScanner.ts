...
/**
 * Forma mínima que `FrameworkScanner` necesita de `indexing.ReuseAnalyzer`
 * (ver ADR-0001, "retiro de fachadas"): `indexing` depende de `workspace`
 * para ubicar el árbol del framework, así que un import inverso hacia
 * `indexing` aquí cerraría un ciclo entre ambos módulos. Quien construye
 * este scanner (composition root) inyecta el `ReuseAnalyzer` real.
 */
export interface ReuseSummaryProvider {
...
export class FrameworkScanner {
    ...
    constructor(private readonly reuseAnalyzer: ReuseSummaryProvider) {}
