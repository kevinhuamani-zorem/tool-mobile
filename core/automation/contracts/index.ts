...
import type { AutomationValidation } from './automationValidation';
export type {
    GenerationRequest,
    MobilePlatform,
    TestPathType,
...
export type { AutomationScenario } from './automationScenario';

/**
 * Fase de modularización (ver ADR-0001): estas capas puras del "vocabulario"
 * de automation viven físicamente en `automation/contracts` porque tanto
 * `generation` como `validation` las consumen y ambos módulos solo pueden
 * alcanzar `automation` a través de esta API pública (nunca de
...
export interface ActionResolution {
    ...
    /**
     * Reemplazo explícito autorizado por QA sobre una clave existente.
     * El selector nunca viene del agente: `sequence` apunta a la evidencia
     * verificada del recording y el generador copia de allí tipo y valor.
     */
    locatorReplacement?: {
        file: string;
        module: string;
...
export type GapResolutionDecision = 'reuse' | 'create' | 'replace-existing'
    | 'resolved' | 'qa-required' | 'unresolved';

/**
 * Alias que el agente puede copiar del `GenerationPlan`. Se acepta únicamente
 * en la frontera JSON y se normaliza antes de llegar al dominio.
 */
export type GapResolutionDecisionInput = GapResolutionDecision
    | 'reuse-existing'
    | 'create-new'
    | 'extend-existing';
...
export interface GapResolution {
    ...
    /**
     * Solo para `replace-existing`. Identifica plataforma y acción grabada que
     * aportan el TypeLocator y selector verificados; el agente no los escribe.
     */
    replacement?: {
        platform: MobilePlatform;
        sequence: number;
