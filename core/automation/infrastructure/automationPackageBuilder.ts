...
function instructions(result: ResolverResult): string {
    ...
    return `# Contrato del agente de automatización\n\n` +
        ...
        `- En deterministic solo escribe query-requests.json o gap-resolutions.json. No edites agent-response.json ni los archivos inmutables del paquete. Si scenarioRows trae wording \"template\", resuélvelo dentro de gherkinResolutions en gap-resolutions.json. Incluye siempre testDesignReview: contrasta objetivo y aceptación con aserciones posteriores de resultado de negocio; verificar que existe el control no demuestra su efecto. Si el status es qa-required, Copilot debe redactar roast directo y obviamente troll: menciona el error concreto, remata por qué no prueba nada y pide corregirlo; sin metáforas vagas, insultos personales ni fallos inventados.\n` +
