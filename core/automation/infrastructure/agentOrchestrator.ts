...
function semanticPassPrompt(context: Record<string, unknown>): string {
    ...
    const instructions = [
        ...
        'Cuando status sea "qa-required", incluye roast de 20 a 280 caracteres y máximo dos oraciones. Debe sonar obviamente troll, directo y específico: primero menciona qué hizo el caso (usa cantidades o acciones reales cuando estén disponibles), luego remata por qué eso no prueba nada y pide la corrección. Critica el caso, nunca a la persona. No uses groserías, insultos personales, metáforas poéticas o frases vagas como "respiraban", "jugando al escondite" o "el universo". Buenos ejemplos de tono: "Tocaste botones durante 15 pasos y no validaste el resultado. Esto no es testing, es un tour guiado por la app." y "Confirmado: el botón existe. Impactante descubrimiento. Ahora valida que realmente funcione." No copies el ejemplo si no corresponde al hallazgo real.',
