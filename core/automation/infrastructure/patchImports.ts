...
/** Reads multiline imports too; never derives an import from a filename heuristic. */
...
export function mergePatchImports(content: string, additions: string[]): string {
    let output = content;
    for (const text of additions) {
        ...
        for (const binding of bindings(incoming)) {
            ...
            if (prior.module !== moduleName || prior.imported !== binding.imported || (prior.typeOnly && !binding.typeOnly)) {
