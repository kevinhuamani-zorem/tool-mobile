...
import { ReuseAnalyzer, SquadReuseCatalog, CodeGraph, importsOf, indexModuleImports, recordedLocator } from '../../indexing';
import {
...
import { frameworkContract, projectPaths, selectFrameworkUser } from '../../workspace';
...
export class DeterministicResolver {
    ...
    resolve(rawScenario: AutomationScenario, options: { memory?: MemoryFragmentsPort } = {}): ResolverResult {
        ...
        const requestedUser = rawScenario.request.dataName?.trim();
        const testUser = selectFrameworkUser(rawScenario.squad, rawScenario.recordingId,
            requestedUser || rawScenario.request.examples?.username || '');
        const normalizedRequest: GenerationRequest = {
            ...
            dataName: testUser.name,
            testDataSelection: testUser.automatic ? { mode: 'automatic', name: testUser.name, file: testUser.file, reason: testUser.reason }
                : rawScenario.request.testDataSelection?.name === testUser.name ? rawScenario.request.testDataSelection : undefined,
        };
        ...
        const inconsistentLocators = rawScenario.actions.filter(step => step.selector && !recordedLocator(step, rawScenario.platform).ok);
        if (inconsistentLocators.length) gaps.push({
            id: 'gap-locator-roundtrip', type: 'missing-selector', blocking: true,
            description: inconsistentLocators.map(step => `Acción ${step.sequence} (${step.selector}): ${recordedLocator(step, rawScenario.platform).reason}`).join(' | '),
            requiredOutput: 'Verifica el tipo y valor originales contra el dispositivo: ID, XPATH, ANDROID con UiSelector o la estrategia iOS correspondiente. El agente no puede sustituir la evidencia del selector.',
        });
        ...
        const createFromPrimary = (
            ...
            const pair = recordedLocator(step, rawScenario.platform);
        ...
        const broken = resolutions
            ...
            .map(resolution => ({
                resolution,
                check: recordedLocator(rawScenario.actions[resolution.sequence - 1] || { selector: resolution.selector }, rawScenario.platform),
            }))
            .filter(entry => !entry.check.ok);
        if (broken.length && !gaps.some(gap => gap.id === 'gap-locator-roundtrip')) {
            gaps.push({
        ...
        const examples: Record<string, string> = {
            ...
            username: normalizedRequest.dataName || '',
        };
