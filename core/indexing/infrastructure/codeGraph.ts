...
/**
 * Forma mínima de una acción grabada que `CodeGraph.query` necesita para
 * buscar y rankear nodos del framework. `automation/contracts/RecordedStep`
 * (estructuralmente compatible) es lo que de hecho se pasa aquí siempre,
 * pero `indexing` no importa `automation` para no cerrar un ciclo entre
 * ambos módulos (ver ADR-0001): `automation/contracts` ya depende de
 * `indexing` para su vocabulario de locators/selectores.
 */
...
/** Copia local de `automation/contracts/recordedStepContext`; ver `QueryableAction`. */
function queryableActionContext(action: QueryableAction): string {
    return String(action.contextHint || action.elementIntent || action.description || '').trim();
}
...
export class CodeGraph {
    ...
    query(input: {
        ...
        const searchWords = words(input.actions.map(action => [
            action.action, action.variableName, queryableActionContext(action)
