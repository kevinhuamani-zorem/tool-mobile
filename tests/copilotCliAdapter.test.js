...
test('adapter corta la sesión cuando la corrección repite los mismos errores (stuck)', async () => {
    ...
    try {
        ...
        assert.equal(killed, true, 'la sesión se corta');
        assert.ok(Date.now() - started < 2_000, 'no espera al plazo de inactividad');
        const trace = fs.readFileSync(path.join(root, 'trace.log'), 'utf8');
        assert.match(trace, /\[output-rejected\]/);
        assert.match(trace, /\[feedback-stuck\] La corrección repite los mismos errores que el feedback anterior \(ronda 2\)/);
