...
test('launcher usa modelo elegido y UUID nuevo sin cambiar permisos', () => {
    ...
    assert.match(args, /\.recorder-copilot-/);
    const firstScript = fs.readFileSync(path.join(root, `.recorder-copilot-${first.sessionId}.sh`), 'utf8');
    assert.match(firstScript, /'--model' 'gpt-5.6-terra'/);
    assert.ok(firstScript.includes(first.sessionId));
    assert.doesNotMatch(firstScript, /--allow-all/);
    assert.match(firstScript, /--allow-tool=shell\(node\)/);
    assert.match(firstScript, /--allow-tool=shell\(python\)/);
    assert.match(firstScript, /--allow-tool=shell\(python3\)/);
    assert.match(firstScript, /--allow-tool=read/);
    assert.match(firstScript, /--allow-tool=write/);
    assert.match(firstScript, /--add-dir/);
    assert.doesNotMatch(firstScript, /--deny-tool=bash|--allow-all-paths|--yolo/);
