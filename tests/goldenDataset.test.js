...
test('equivalent golden examples remain accessible in full beyond former byte and count caps', t => {
    const { selectGoldenExamples, writeGoldenRoleExamples } = require('../dist/core/automation');
    const f = appliedPackageFixture(t); withGoldenRoot(t, f);
    const { projectPaths } = require('../dist/core/workspace'); const before = projectPaths.frameworkRoot;
    projectPaths.frameworkRoot = f.frameworkRoot; t.after(() => { projectPaths.frameworkRoot = before; });
    ...
    const prepared = writeGoldenRoleExamples(pkg, stage, 'behavior-author', 1);
    const payload = JSON.parse(fs.readFileSync(prepared.file));
    assert.equal(payload.examples.length, 1); assert.equal(prepared.references.length, 1);
    assert.equal(payload.retrieval.candidateCases, 6); assert.deepEqual(payload.skipped, []);
    const variants = prepared.session.request({ id: 'variants', operation: 'variants', groupId: payload.retrieval.catalog[0].groupId }).data.references;
    assert.equal(variants.length, 6);
    for (const reference of variants) {
        const response = prepared.session.request({ id: 'read-' + reference.caseId, operation: 'example', goldenId: reference.goldenId, versionHash: reference.versionHash });
        assert.equal(response.data.files.find(file => file.layer === 'steps').content, content);
    }
});
